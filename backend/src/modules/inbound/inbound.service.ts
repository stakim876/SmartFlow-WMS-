...
export const inboundListQuerySchema = z.object({
...
type InboundWithRelations = NonNullable<Awaited<ReturnType<typeof inboundRepository.findById>>>;

function toInboundResponse(order: InboundWithRelations) {
  return {
    id: order.id,
...
export const inboundService = {
  async list(query: z.infer<typeof inboundListQuerySchema>) {
    const { page, limit, skip } = getPagination(query);
    const [items, total] = await inboundRepository.findMany({
      skip,
      take: limit,
      status: query.status,
    });
    ...
    return {
      items: items.map(toInboundResponse),
      meta: buildPaginationMeta(total, page, limit),
    };
  },

  async create(input: z.infer<typeof createInboundSchema>) {
    ...
    const order = await inboundRepository.create({
    ...
    return toInboundResponse(order);
  },

  async approve(id: string) {
    const order = await inboundRepository.findById(id);
    if (!order) {
      throw new AppError(404, '입고 전표를 찾을 수 없습니다.', 'INBOUND_NOT_FOUND');
    }
    if (order.status !== OrderStatus.PENDING) {
      throw new AppError(400, '승인 대기 상태의 전표만 승인할 수 있습니다.', 'INVALID_STATUS');
    }

    const updated = await inboundRepository.update(id, {
      status: OrderStatus.APPROVED,
      approvedAt: new Date(),
    });

    void notificationService.refreshAllUsers();
    return toInboundResponse(updated);
  },

  async complete(
    id: string,
    input: z.infer<typeof completeInboundSchema>,
    userId?: string,
  ) {
    const order = await inboundRepository.findById(id);
    if (!order) {
