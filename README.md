...
React 프론트와 Express API를 npm workspaces로 묶은 모노레포입니다.
...
`backend/.env`는 `backend/.env.example`을 복사해서 사용합니다.  
MySQL은 Docker로 실행합니다.
...
로컬 5173 포트를 다른 앱이 쓰는 경우가 있어 5175를 사용합니다.
...
| 역할 | 설명 |
|------|------|
| ADMIN | 전체 관리, 승인, 직원 및 창고 설정 |
| STAFF | 입출고, 재고, 발주 등 일상 운영 |
| VIEWER | 조회와 Excel 다운로드만 가능 |

직원 등록 화면에서 역할을 지정합니다.

## 주요 기능

상품과 재고: 창고/로케이션별 조회, 수량 조정, 로케이션 이동, 이력에 작업자 기록

입고와 출고: 전표 등록 후 승인, 완료 처리. 상태별 목록 필터 지원

발주: 승인 후 입고 전환으로 입고 전표 생성. 입고 완료 시 발주도 완료 처리

창고와 로케이션: 창고 및 보관 위치 마스터 관리
...
1. 발주 등록 후 관리자가 승인
2. 승인된 발주에서 입고 전환
3. 입고 관리에서 승인 후 로케이션 지정, 완료
4. 재고가 반영되고 발주 상태가 완료로 변경
...
Vitest로 backend, frontend 단위 테스트를 실행합니다.
...
자세한 내용은 [docs/architecture.md](docs/architecture.md), [docs/folder-structure.md](docs/folder-structure.md)를 참고하세요.
