...
import { CreateOrderItem } from '@/features/outbound/api/outbound';
import { COMMON, OUTBOUND } from '@/shared/constants/labels';
import styles from './OutboundFormModal.module.css';
...
export function OutboundFormModal({ onClose, onSave }: OutboundFormModalProps) {
  ...
  return (
    <div className={styles.overlay} onClick={onClose}>
      <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
        <h2 className={styles.title}>{OUTBOUND.formTitle}</h2>
        <form onSubmit={handleSubmit} className={styles.form}>
          {error && <div className={styles.error}>{error}</div>}
          <label className={styles.label}>
            {OUTBOUND.partnerOptional}
            <select
              value={partnerId}
          ...
          {items.map((item, index) => (
            <div key={item.key} className={styles.itemRow}>
              <select
                ...
                required
              >
                <option value="">{OUTBOUND.selectProduct}</option>
                {products.map((p) => (
                  <option key={p.id} value={p.id}>
                    {p.sku} - {p.name}
                  </option>
              ...
              <select
                ...
                className={styles.input}
              >
                <option value="">{OUTBOUND.locationOnComplete}</option>
                {locations.map((l) => (
